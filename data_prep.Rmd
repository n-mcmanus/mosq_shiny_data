...
*NOTE:* Trying to crop/mask them by zipcode, then stack, to see if that runs faster in Shiny.
```{r}
path23 = here("data/water/p042_masked_merged/2023//")
path22 = here("data/water/p042_masked_merged/2022//")
...
rasts_df = data.frame(rasts = c(list.files(path22,pattern="kern",full.names=FALSE),
                                list.files(path23,pattern="kern",full.names=FALSE))) %>% 
  ## Add dates, then remove select rasters based on them
  mutate(dates(rasts),
         rasts = c(list.files(path22,pattern="kern",full.names=TRUE),
                          list.files(path23,pattern="kern",full.names=TRUE))) %>%  
  filter(!date %in% c("2022-03-10","2022-06-06","2023-04-22",
                      "2023-05-08","2023-05-16","2023-06-09","2023-07-03")) %>% 
  mutate(date = gsub("-", "", date))


## Df of all zipcodes
zips_df <- read_sf(here("data/zipcodes/kern_zips.shp")) %>% 
  dplyr::select(zipcode) %>% 
  as.data.frame() %>% 
  dplyr::select(!geometry) %>% 
  mutate(path = here("data/zipcodes/kern_zips.shp"))
  
# crop_to_zip <- function(zipcode, rast) {
...
test <- function(zipcode, path, rasts_df) {
  
  for(i in 1:nrow(rasts_df)) {
    ## Read in each raster and zipcode as vect
    r <- rast(rasts_df$rasts[1])
    values(r)[values(r) == 0] <- NA
    
    zip <- read_sf(zips_df$path[1]) %>% 
      filter(zipcode == zips_df$zipcode[1]) %>% 
      vect() %>% 
      project(y=crs(r))
    ...
    m <- leaflet() %>%
      ...
      addRasterImage(r_crop, colors = "dodgerblue4", project =T)
    
    saveWidget(m, "temp.html", selfcontained = FALSE)
    webshot("temp.html", file = "plottest.png",
            cliprect = "viewport")
    
  }
}

path = paste0(here("data/water/zips//"), 
                                       sprintf("zip_%02s_", zips_df$zipcode[1]),
                                       sprintf("%02s.png", rasts_df$date[1]))
...
for (i in 1:nrow(df)) {
  r <- rast(df$rasts[i])
  zip <- read_sf(here("data/zipcodes/kern_zips.shp")) %>% 
    filter(zipcode == zips_df$zipcode[i])
  
  
  m <- leaflet() %>% 
    addProviderTiles(providers$Esri.WorldImagery) %>%
    addRasterImage(r[[2]], colors = "dodgerblue4",
                   project =F)

  saveWidget(m, "temp.html", selfcontained = FALSE)
  webshot("temp.html", file = here("testplot2.png"), cliprect = "viewport")
}

  # ## old test w/one zip
  # filter(zipcode == "93263") %>% 
  # vect() %>% 
  # project(y=crs(rast1))


## Read in each, crop and mask to zip
for (i in 1:nrow(df)) {
  r = rast(df$rasts[i]) %>% 
    crop(x=., y=zip, mask = TRUE)
  names(r) = paste0("rast_", df$date[i])
  values(r)[values(r) == 0] <- NA
...
s <- sprc(rast1, rast2, rast3, rast4, rast5,
          rast6, rast7, rast8, rast9)
...
## Stack and export
stack = c(rast1, rast2, rast3, rast4, rast5,
          rast6, rast7, rast8, rast9)

writeRaster(stack, here("data/water/zips/zip93269_2023_stack.tif"),
            overwrite = TRUE)
```
...
## Make a gif
library(magick)
png_files <- c("testplot.png", "testplot2.png")
GIF.convert <- function(x, output = "animation.gif") {
  image_read(x) %>% 
    image_animate(fps=1) %>% 
    image_write(output)
}
GIF.convert(png_files)
```
...
data_filtered <- water_zip_df %>% 
  filter(zipcode == user_input, 
         date >= "2020-03-01" & date <= "2020-07-31")


ggplot(data_filtered, aes(x = date, y = acres_int)) +
