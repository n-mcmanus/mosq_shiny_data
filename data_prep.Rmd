...
```{r}
pathIn = here("data/water/p042_masked_merged/2023//")
...
df = data.frame(rasts = list.files(pathIn, pattern="kern", full.names=FALSE)) %>%
  ## Add dates, then remove select rasters based on them
  mutate(dates(rasts),
         rasts = paste0(pathIn, rasts)) %>%  
  filter(!date %in% c("2023-04-22","2023-05-08","2023-05-16",
                      "2023-06-09","2023-07-03")) %>% 
  mutate(date = gsub("-", "_", date))


## Test with zipcode 93263
