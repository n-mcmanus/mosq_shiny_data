...
The mean daily temperature by zip code was extracted from the PRISM dataset using a Google Earth Engine script. This was exported as a CSV with variables for date ("imageID"), zip code ("GEOID10"), and daily mean temperate ("mean"). We'll read in and clean up the CSV, as well as determine if each observation falls w/in the optimal range for WNV transmission in *Culex tarsalis* mosquitoes. This is defined as between 22.9-25.9C (optimum temp of 23.9 w/95% CI) (Shocket et al., 2020)
```{r}
...
temp2 <- temp %>% 
  ...
  mutate(cxTar_opt = case_when(tmean_c >= 22.9 & tmean_c <= 25.9 ~ "TRUE",
                                .default = "FALSE")) %>% 
  ## set T/F as factor for consistent graphic
  mutate(cxTar_opt = fct_relevel(cxTar_opt, levels = c("TRUE", "FALSE")))
...
ggplot()+
  ...
  geom_point(data = cases_zip_yr, 
             aes(x=woy, y = avgMIR),
             color = "sienna2", size = 3, alpha = 0.6) +
  geom_line(data = cases_zip_yr, 
             aes(x=woy, y = avgMIR),
            color = "sienna4") +
  ## avg zipcode
...
kern_avgAbund <- mean(abund$mos_per_trap_night, na.rm = TRUE)
...
ggplot()+
  ## avg kern
  geom_hline(yintercept = kern_avgAbund,
            color = "black", linetype = "longdash", linewidth = 0.8) +
  annotate("text", y = (kern_avgAbund+0.07), x = abund_cust$date[1],
           hjust = 0,
           label = "Average abundance across Kern county",
           size = 3,
           fontface = "bold") +
   # avg zipcode
