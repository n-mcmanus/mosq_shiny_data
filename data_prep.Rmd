...
## Save Kern vect for leaflet too
...
**NOTE:** Still waiting on some mask files from Dan. Will run when available.
```{r}
## Create paths and lists of water rasters and masks
path35 <- here('data/water/Landsat_dan/p042r035//')
path36 <- here('data/water/Landsat_dan/p042r036//')
rNames35 <- list.files(path=path35, pattern="_T.TIF")
mNames35 <- list.files(path=path35, pattern="QA_PIXEL.TIF")
rNames36 <- list.files(path=path36, pattern="_T.TIF")
mNames36 <- list.files(path=path36, pattern="QA_PIXEL.TIF")

## Extract dates for each based on LANDSAT naming convention.
## Same dates for both row 35 and 36 images.
## Split up name by underscores, return as own column in df
dates <- str_split(rNames35, "_", simplify = TRUE) %>%
  as.data.frame() %>%
...
r <- rast(paste0(path35, rNames35[1]))
names(r) <- "rast"
...
for (i in 1:length(rNames35)) {
  ## raster and mask for row 35 -------------  
  r35 <- rast(paste0(path35, rNames35[i]))
  names(r35) <- "rast"
  m35 <- rast(paste0(path35, mNames35[i]))
  names(m35) <- "mask"
  ...
  r36 <- rast(paste0(path36, rNames36[i]))
  names(r36) <- "rast"
  m36 <- rast(paste0(path36, mNames36[i]))
  names(m36) <- "mask"
  ## mask values of 1 are NA
  m36[mask == 1] <- NA
  ## mask raster 36
  ...
  ## find zonal stats using fxn created above
  r_stats <- water_stats(m, kern_zips, dates[i,])
...
*NOTE:* This may be replaced by another transmission raster in the future.
```{r}
## Read in data
...
*NOTE:* A fuller data set is currently being wrangled and standardized for effort by Andy. Well be updated in future.
```{r}
