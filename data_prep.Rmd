...
Here we'll perform some simple "raster math" to get a better idea of how frequently certain areas of Kern County are flooded.
```{r}
## List all rasters
path23 = here("data/water/p042_masked_merged/2023//")
path22 = here("data/water/p042_masked_merged/2022//")
...
rasts_list <- terra::rast(
  lapply(rasts_df$rasts, function(x) {
    r <- rast(x) %>%
      resample(y = sample_r, 
...
## Add stack, then replace 0 with NA
rasts_sum <- sum(rasts_list)
rasts_sum_na <- classify(rasts_sum, cbind(0, NA))

## Save
writeRaster(rasts_sum_na, here("data/water/summed_water_2022_2023.tif"))


## Save 90m version
r <- rast(here("data/water/summed_water_2022_2023.tif"))
r_agg <- aggregate(r, fact = 3, fun = "modal")
writeRaster(r_agg, here("data/water/summed_water_2022_2023_90m.tif"))
...
```{r}
wnv <- read_csv(here("data/traps/plotting/wnvMIR_plotting.csv"))
...
posZips <- unique(wnvDates$zipcode)

zipInput <- "93311"

ifelse(zipInput %in% posZips, "yes", "no")


wnvZips <- zips_sf %>% 
      dplyr::filter(zipcode %in% wnvDates$zipcode)
```
