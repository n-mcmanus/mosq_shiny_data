...
#### Merge images
Our AOI is split between two Landsat images (rows 35 and row 36 within path 42). First we'll merge and export them by each date. Then, we'll merge these new rasters and merge them all to determine a common extent. Finally, we'll change the extent of each raster to that of the merged, stack it, and save for use as gif in shiny.
```{r}
...
waterMerge <- function(rast35, mask35, rast36, mask36, date, pathOut) {
...
#### Zonal stats
Here we determine how much standing water is present within each zip code. This information will be exported as a .csv and used for the Shiny app. To do this, we'll need to first ensure that our `kern_zips.shp` are the same crs as the water rasters. Then we'll use a function to loop through all the water rasters within a given directory.
```{r}
## Create paths for water rasters and masks
pathIn <- here('data/water/p042_masked_merged/2022//')
...
df <- data.frame("rasts" = list.files(pathIn, pattern=".tif")) %>% 
  ## extract date from r35
  mutate(dates(rasts),
         rasts = paste0(pathIn, rasts))
...
waterStats <- function(rasts, zip_vect, date) {
  
  r = terra::rast(rasts)
 
  zonalStat <- terra::zonal(r, zip_vect, fun = 'sum', na.rm = TRUE)
...
waterStats_df <- pmap_dfr(df, waterStats, zip_vect = kern_zips, .progress = TRUE)

## export as .csv
write_csv(waterStats_df, here('data/water/water_acre_zipcode_2022.csv'))
```
...
rasts = list.files(here("data/water/masked_merged/"),
                   pattern = "p42_merged_",
                   full.names = T)
...
s <- sprc(rast1, rast2, rast3, rast4, rast5, rast6, rast7,
          rast8, rast9, rast10, rast11, rast12, rast12, rast14)
m <- merge(s)
...
writeRaster(stack, here("data/water/masked_merged/p42_merged_stack.tif"))


```

Water rasters for gif. Must be same extent, resolution, and number of rows/columns. First we'll merged and export all the path42 rasters by date. Then, we'll read in these new rasters and merge them to determine the extent that they all should have in common. Finally, we'll change the extent of each raster to that of the merged, stack it, and save.
```{r}
## Create paths for water rasters and masks
...
## One df with all files and dates
waterInput_df <- data.frame("rast35" = list.files(path35, 
                                                  pattern="_T.TIF"),
                            "mask35" = list.files(path35, 
                                                  pattern = "QA_PIXEL.TIF"),
                            "rast36" = list.files(path36, 
                                                  pattern="_T.TIF"),
                            "mask36" = list.files(path36,
                                                  pattern="QA_PIXEL.TIF")) %>%
  ## extract date from r35
  mutate(dates(rast35))

## Function to perform zonal stats and output results
waterMerge <- function(rast35, mask35, rast36, mask36, date) {
  ...
  # assign(paste0("rast", date), m)
  
  writeRaster(m, paste0(here("data/water/masked_merged//"), "p42_merged_", date, ".tif"))
}

## Run fxn over df of variables
pmap(waterInput_df, waterMerge, .progress = TRUE)
...
test = raster::stack(here("data/water/masked_merged/p42_merged_stack.tif"))
raster::animate(test)
```
