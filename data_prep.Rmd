...
First we'll reproject and crop Landsat images to Kern county. Next, we'll run zonal stats to determine how much standing water is present in each zip code. This information will be exported as a .csv and used for the Shiny app.
...
water_zips <- water_zonal %>% 
  ...
  ## clean up names
  rename(water_acre = LC09_CU_003011_20230625_20230701_02_DSWE_Binary2_T) %>% 
  ## convert values from "units" acres
  ## w/30m resolution, each unit is 900 m^2
  ## 4046.86 m^2 are in one acre
  mutate(water_acre = water_acre * 900 / 4046.86) %>% 
  ## finally add date information
  mutate(date = ymd("2023-06-25"))

## export as .csv
write_csv(water_zips, here('data/water/water_acre_zipcode.csv'))
```


Here we'll find an average WNV transmission risk for each zipcode
```{r}
...
test <- risk_zonal_zips %>% 
  filter(zipcode == 93307)

if (is.na(test$trans_risk)) {
  print("Not available")
} else {print(test$trans_risk)}
```
