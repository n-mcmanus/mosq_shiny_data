...
rasts <- list.files(path23, pattern = "kern", full.names=FALSE)
...
rasts_df = data.frame(rasts = rep(rasts, length(zips_list)),
  ...
  mutate(dates(rasts),
         date = as.character(date),
         rasts = rep(list.files(path23,pattern="kern",full.names=TRUE),
                     length(zips_list))) %>%  
...
p<- ggplot(data_filtered, aes(x = date, y = acres_int)) +
  geom_point(aes(group=seq_along(date)), color = "dodgerblue3", size = 3, alpha = 0.6) +
  geom_point(color = "dodgerblue3", size = 4) +
  geom_line(linewidth = 0.7, color = "dodgerblue4") +
...
p
animate(p, fps = 50, nframes = 700, end_pause = 100)

anim_save("test.gif", animate(p, fps = 50, nframes = 700, end_pause = 100))



p + geom_line() +
  geom_point(aes(group = seq_along(date)), color = "dodgerblue3", size = 4) +
  transition_reveal(date)
