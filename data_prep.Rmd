...
waterZonal <- function(rastPath, vect) {
    ...
    df <- data.frame("rasts" = list.files(rastPath, pattern="merged")) %>% 
      ## extract date from r35
      mutate(dates(rasts),
...
rasts = list.files(here("data/water/p042_masked_merged/2022"),
                   pattern = "p42_merged_",
                   full.names = T)
...
writeRaster(stack, here("data/water/p042_masked_merged/p42_2022_stack.tif"))
